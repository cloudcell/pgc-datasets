#!/usr/bin/env python3
"""
Dataset viewer for PGC benchmarking system.
This script provides a GUI for visualizing and analyzing datasets.
"""
...
class DatasetSelector:
            ...
            try:
                if dataset_path.endswith('.h5'):
                    with h5py.File(dataset_path, 'r') as f:
                        ...
                        # Try to convert features/labels to torch if possible
                        try:
                        ...
                        try:
                            ...
                            pass
                        # Metadata: read as dict from group/attrs
                        metadata = {}
                        if 'metadata' in f:
                            ...
                            for k, v in meta_group.attrs.items():
                                metadata[k] = v
                            # datasets in metadata group
                            for k in meta_group:
                                try:
                                    metadata[k] = np.array(meta_group[k])
                                except Exception:
                                    metadata[k] = str(meta_group[k][:])
                        data = {'features': features, 'labels': labels, 'metadata': metadata}
                else:
    ...
    def show_viewer(self, data, dataset_path):
        ...
        # Extract metadata if available, otherwise use None
        metadata = data.get('metadata', None)
        
        # Show viewer
        DatasetViewer(self.root, data['features'], data['labels'], dataset_path, metadata)

class DatasetViewer:
    def __init__(self, root, features, labels, dataset_path, metadata=None):
        import matplotlib
...
"""
    ...
    def show_metadata_dialog(self):
        """Display dataset metadata in a dialog window"""
        if self.metadata is None:
            # Show a message if no metadata is available
            tk.messagebox.showinfo("No Metadata", "No metadata is available for this dataset.")
            return
            
        # Create dialog window
        dialog = tk.Toplevel(self.root)
        dialog.title("Dataset Metadata")
        dialog.transient(self.root)
        dialog.grab_set()
        ...
        # Create text widget with scrollbar for displaying metadata
        ...
        text.insert(tk.END, "Dataset Metadata:\n\n")
        
        # Format metadata as key-value pairs
        for key, value in self.metadata.items():
            # Format special cases
            if key == 'feature_dim' and isinstance(value, tuple):
                value_str = f"{value}"
            ...
            text.insert(tk.END, f"{value_str}\n")
    ...
    def show_ascii_reference(self):
        from pgc_data_lib.ascii_tables import create_ascii_reference_dialog
        create_ascii_reference_dialog(self.root)
    ...
    def show_current_sample(self):
        ...
        # Update features text display using the exact same approach as the hex viewer
